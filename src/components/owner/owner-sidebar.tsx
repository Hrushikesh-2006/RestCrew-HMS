...
const ownerNavItems: NavItem[] = [
  { label: 'Dashboard', icon: LayoutDashboard, href: '/owner/dashboard', gradient: 'from-purple-500 to-pink-500' },
  { label: 'Rooms', icon: DoorOpen, href: '/owner/rooms', gradient: 'from-cyan-500 to-blue-500' },
  { label: 'Students', icon: Users, href: '/owner/students', gradient: 'from-green-500 to-emerald-500' },
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 glass-dark z-50 flex items-center justify-between px-4">
        <button
          ...
          className="p-2 rounded-xl hover:bg-purple-500/10 transition-colors"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold gradient-text text-sm">RestCrew</span>
        </div>
        <div className="w-9" />
      </div>
      ...
      <AnimatePresence>
        <motion.aside
          initial={{ x: -280 }}
          animate={{ x: isOpen ? 0 : (typeof window !== 'undefined' && window.innerWidth >= 1024 ? 0 : -280) }}
          className={cn(
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <motion.div
                  ...
                  className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 via-pink-500 to-orange-500 flex items-center justify-center shadow-lg shadow-purple-500/20"
                >
                  <Building2 className="w-5 h-5 text-white" />
                </motion.div>
                <div>
                  <h1 className="text-base font-bold gradient-text">RestCrew</h1>
                  <p className="text-[10px] text-muted-foreground">Owner Panel</p>
                </div>
              </div>
              <button
                onClick={() => setIsOpen(false)}
              ...
              <div className="glass rounded-xl p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">{owner.hostelName}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    ...
                    'hover:bg-white/5',
                    isActive && 'bg-gradient-to-r from-purple-500/20 to-transparent border-l-2 border-purple-500'
                  )}
                >
                  <div className={cn(
